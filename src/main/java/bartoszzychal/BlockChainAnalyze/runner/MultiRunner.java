...
public class MultiRunner extends AbstractRunner {
	private static final Logger log = Logger.getLogger(MultiRunner.class);
	...
	public static void main(String[] args) {
		...
		final MultiRunner runner = new MultiRunner();
		Integer sampleStart = null;
		int transactionStart = 16;
		int countOfTransactions = 15;
		int countOfblock = 10000;
		runner.run(connectionsLimit, sampleStart, transactionStart, countOfTransactions, countOfblock);
	}

	private void run(Long connectionsLimit, Integer sampleStart, Integer transactionStart, Integer countOfTransactions,Integer countOfblock) {
		...
		ContextPropagatingThreadFactory contextPropagatingThreadFactory = new ContextPropagatingThreadFactory("Runner");
		...
		for (int i = startHash; i < range; i++) {
			...
			if (CollectionUtils.isNotEmpty(transactions) && transactions.size() >= countOfTransactions + 1) {
				// get hashes
				...
				final BlocksConnectionFinder blocksConnectionFinder = new BlocksConnectionFinder(transactionConnectionInput, blockchainReader, countOfblock);
				Thread thread = contextPropagatingThreadFactory.newThread(blocksConnectionFinder);
				thread.setName(i + ". " + block.getHashAsString());
				threads.add(thread);
				thread.start();
			}
		}
